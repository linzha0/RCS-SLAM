...
### Usages:
# python3 /home/lin/develop/ros/soslab_ws/src/slam/RCS-SLAM/ajh_swarm_slam/test/scripts/plot_20bots.py
...
class evaluateAllTrajectory:
    ...
    def parse_arguments(self):
        ...
        for i in range(1, self.robot_num+1):
            ### Get the file
            filepath = self.truth_path + 'Robot'+ str(i) + '-odom.csv'
        ...
        for i in range(1, self.robot_num+1):
            ### Get the file
            filepath = self.odom_path + 'Robot'+ str(i) + '-noisy_odom.csv'
            is_exist = os.path.isfile(filepath)
            if is_exist==False:
                print(f'file: {filepath}, exist: {"True" if is_exist else "False"}')
                continue
